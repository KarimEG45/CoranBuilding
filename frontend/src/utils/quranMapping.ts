...
const surahs: SurahInfo[] = [
    { number: 1, name: "Al-Fariha", startPage: 1 },
    { number: 2, name: "Al-Baqara", startPage: 2 },
    { number: 3, name: "Ali 'Imran", startPage: 50 },
    ...
    { number: 23, name: "Al-Mu'minun", startPage: 341 },
    ...
    { number: 73, name: "Al-Muzzammil", startPage: 573 },
...
export function getSurahForPage(page: number): string {
    if (page < 1 || page > 604) return "Hors Quran";
    // Find the last surah that starts on or before this page
    const surah = [...surahs].reverse().find(s => s.startPage <= page);
    return surah ? `Sourate ${surah.name}` : "Sourate Inconnue";
}
